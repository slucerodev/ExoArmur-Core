...
from exoarmur.replay.canonical_utils import stable_hash, to_canonical_event
...
def replay_audit_stream(audit_stream_id: str):
    ...
    canonical_records = _convert_to_canonical_events(audit_records)
    replay_report = ReplayEngine(audit_store={audit_stream_id: canonical_records}).replay_correlation(audit_stream_id)
    replay_timeline = [
        {
    ...
    replay_result = {
        ...
        "deterministic_hash": stable_hash(
            json.dumps([event.model_dump(mode="json") for event in canonical_records], sort_keys=True, separators=(",", ":"))
        ),
    }
