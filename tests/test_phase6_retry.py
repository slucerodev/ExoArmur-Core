...
class FailingOperation:
    ...
    def __init__(self, fail_count=2, exception_type=Exception):
    ...
    async def execute(self):
        ...
        if self.attempts <= self.fail_count:
            raise self.exception_type(f"Attempt {self.attempts} failed")
        return "success"
...
async def test_retry_with_failures():
    ...
    assert failing_op.attempts == 3, "Should have attempted 3 times (2 fails + 1 success)"
...
async def test_retry_exhaustion():
    ...
    failing_op = FailingOperation(fail_count=10)  # More than max attempts
...
async def test_retry_with_idempotency():
    ...
    assert failing_op.attempts == 3, "Should have attempted 3 times"
    
    # Reset operation counter
    failing_op.attempts = 0
    ...
    assert failing_op.attempts == 0, "Should not execute operation again"
...
async def test_retry_decorator():
    ...
    assert result == "success", "Decorated function should succeed after retries"
